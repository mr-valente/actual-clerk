...
- The morning report's header is a fixed, user-named string and every figure lives in the body. A header that changes with the numbers cannot be recognised at a glance, and a body that reports everything is one nobody reads — so each block is individually switchable, including one opt-in list containing every monitored bank-linked account balance, while the alert priority is derived from the state itself and is not affected by what is displayed. The body uses labelled, Markdown-compatible lists that render richly in ntfy's web app and remain readable as plain text in phone clients.
